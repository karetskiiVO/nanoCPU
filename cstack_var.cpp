...
#if HASHFLAG
    stk->hash = stackHash(stk->arr, stk->size * sizeof(Elem_tv));
#endif

    return stackCtor_(stk, len);
}

int stackPush_ (Stack* stk, Elem_tv newElem) {
    if (stk == NULL) {
        return 1 << NULL_PTR_TO_STACK;
    }

#if HASHFLAG
    uint64_t bufhash = stackHash(stk->arr, stk->size * sizeof(Elem_tv));
...
int stackPop_ (Stack* stk, Elem_tv* pastElem) {
    if (stk == NULL) {
        return 1 << NULL_PTR_TO_STACK;
    }
...
int stackCtor_ (Stack* stk, const size_t len) {    
    ...
    if (stk->arr == NULL) {
        stk->arr = (Elem_tv*)malloc(len * sizeof(Elem_tv));
        ...
        stk->arr = (Elem_tv*)realloc(stk->arr, len * sizeof(Elem_tv));
        CHECK(stk->errors, stk->arr, NULL_PTR_TO_STACK);
